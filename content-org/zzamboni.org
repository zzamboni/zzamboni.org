...
In the [[/post/just-enough-lua-to-be-productive-in-hammerspoon-part-2/][next installment]], we will dive into Lua's types and data structures. In the meantime, feel free to explore and learn on your own. If you need more information, I can recommend the following resources, which I have found useful:
...
{{% tip %}}
If you haven't already, be sure to read [[/post/just-enough-lua-to-be-productive-in-hammerspoon-part-1/][the first installment of this series]] to learn about the basic Lua concepts.
{{% /tip %}}
...
The common way of defining a new module in Lua is to create an empty table, and populate it with functions or variables as needed. For example, let's put our [[file:%7B%7B%3C%20relref%20%222017-10-21-just-enough-lua-to-be-productive-in-hammerspoon-part1.md#functions%22%20%3E%7D%7D][double-click generator]] in a module. Create the file =~/.hammerspoon/doubleclick.lua= with the following contents:
...
Lua includes the {{{luadoc(string,6.4)}}} library to implement common string manipulation functions, including pattern matching. All of these functions can be called either as regular functions, with the string as the first argument, or as method calls on the string itself, using the colon syntax (which, as we saw [[file:%7B%7B%3C%20relref%20%222017-10-21-just-enough-lua-to-be-productive-in-hammerspoon-part1.md#dot-vs-colon-method-access-in-lua%22%20%3E%7D%7D][before]], gets converted to the same call). For example, the following two are equivalent:
