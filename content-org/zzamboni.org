...
- In your =rc.elv= (see [[/post/elvish-an-awesome-unix-shell/][mine]] as an example), add the following to load the =bang-bang= module and to set up the appropriate keybindings:
