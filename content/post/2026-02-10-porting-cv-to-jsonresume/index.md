...
featureimage = "img/generated/2026-02-10-porting-cv-to-jsonresume.webp"
toc = true
+++
...
{{< tip >}}
I've now released [resume-toolkit](https://github.com/zzamboni/resume-toolkit), which vastly improves on what's described in this post, and packages it as a standalone tool. See [resume-toolkit: from personal CV pipeline to reusable toolkit]({{< relref "2026-03-27-from-personal-cv-pipeline-to-reusable-toolkit" >}}) for more details, or keep reading to learn how the whole thing started.
{{< /tip >}}
...
For publications I tried to use Typst as well, since it has built-in support for [importing BibTeX files](https://typst.app/docs/reference/model/bibliography/). This worked well with a single bibliography, but I wanted to keep my [publications list](https://zzamboni.org/vita/publications/) structured by type. In the end I decided to stay with LaTeX and used a small wrapper so the output matches the HTML (**Update:** I've also eventually fixed this, processing the entire PDF generation with Typst. This makes the PDF rendering much faster, more predictable and consistent. Again, see [resume-toolkit: from personal CV pipeline to reusable toolkit]({{< relref "2026-03-27-from-personal-cv-pipeline-to-reusable-toolkit" >}}) for more details):
